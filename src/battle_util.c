        ...
        case ENDTURN_WEATHER_DAMAGE:
            ...
            if (!IsBattlerAlive(battler) || !WEATHER_HAS_EFFECT || ability == ABILITY_MAGIC_GUARD)
            {
                ...
                break;
                  ...
                  && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_UTILITY_PACK)
            {
                gBattleScripting.battler = battler;
    ...
    do
    {
        ...
        case CANCELLER_POWDER_MOVE:
            if ((gMovesInfo[gCurrentMove].powderMove) && (gBattlerAttacker != gBattlerTarget))
            {
                ...
                else if (GetBattlerHoldEffect(gBattlerTarget, TRUE) == HOLD_EFFECT_UTILITY_PACK)
                {
                    RecordItemEffectBattle(gBattlerTarget, HOLD_EFFECT_UTILITY_PACK);
                    gLastUsedItem = gBattleMons[gBattlerTarget].item;
                    effect = 1;
                }
    ...
    case ITEMEFFECT_ON_SWITCH_IN_FIRST_TURN:
        if (!gSpecialStatuses[battler].switchInItemDone)
        {
            ...
            case HOLD_EFFECT_AIR_BALLOON:
                ...
                break;
            case HOLD_EFFECT_UTILITY_PACK:
                effect = ITEM_EFFECT_OTHER;
                gBattleScripting.battler = battler;
                BattleScriptPushCursorAndCallback(BattleScript_UtilityPackMsgIn);
                RecordItemEffectBattle(battler, HOLD_EFFECT_UTILITY_PACK);
                break;
        ...
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
        {
            ...
            case HOLD_EFFECT_UTILITY_PACK:
                if (TARGET_TURN_DAMAGED)
                {
                    effect = ITEM_EFFECT_OTHER;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_UtilityPackMsgBreaks;
                }
                break;
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, bool32 considerInverse)
{
    ...
    if (holdEffect == HOLD_EFFECT_AIR_BALLOON)
        return FALSE;
    if (holdEffect == HOLD_EFFECT_UTILITY_PACK)
        return FALSE;
    if ((AI_DATA->aiCalcInProgress ? AI_DATA->abilities[battler] : GetBattlerAbility(battler)) == ABILITY_LEVITATE)
        return FALSE;
    if ((AI_DATA->aiCalcInProgress ? AI_DATA->abilities[battler] : GetBattlerAbility(battler)) == ABILITY_BOOSTED_MOTOR)
        return FALSE;
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities && defAbility == ABILITY_BOOSTED_MOTOR)
        {
            gLastUsedAbility = ABILITY_BOOSTED_MOTOR;
            gMoveResultFlags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
            gLastLandedMoves[battlerDef] = 0;
            gBattleCommunication[MISS_TYPE] = B_MSG_GROUND_MISS;
            RecordAbilityBattle(battlerDef, ABILITY_BOOSTED_MOTOR);
        }
    }
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (moveType == TYPE_GROUND && abilityDef == ABILITY_LEVITATE && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
        if (moveType == TYPE_GROUND && abilityDef == ABILITY_BOOSTED_MOTOR && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
    ...
    if (moveType != TYPE_MYSTERY)
    {
        ...
        if ((modifier <= UQ_4_12(1.0)  &&  abilityDef == ABILITY_WONDER_GUARD)
         ...
         || (moveType == TYPE_GROUND   && (abilityDef == ABILITY_LEVITATE
                                       ||  abilityDef == ABILITY_BOOSTED_MOTOR
...
static void TryToSetUpCombinedMove(void)
{
    ...
    static const struct CombinedMove sCombinedMoves[] = {
        ...
        {MOVE_RAZOR_WIND, MOVE_GUST, MOVE_AIR_CUTTER, STRINGID_WINDBECAMEAIRCUTTER},
        {MOVE_POISON_GAS, MOVE_GUST, MOVE_TOXIC_GAS, STRINGID_WINDBECAMETOXICGAS},
        {MOVE_AQUA_JET, MOVE_ICE_BEAM, MOVE_FROST_JET, STRINGID_AQUABECAMEFROSTJET},
    };
