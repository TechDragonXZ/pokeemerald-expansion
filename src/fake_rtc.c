...
// Edit the value after ? of GEN_CUSTOM to edit the custom ratio. 
u32 FakeRtc_GetSecondsRatio(void)
{
    return (OW_ALTERED_TIME_RATIO == GEN_8_PLA)   ? 60 :
           (OW_ALTERED_TIME_RATIO == GEN_9)       ? 20 :
           (OW_ALTERED_TIME_RATIO == GEN_CUSTOM)  ? 90 :
           (OW_ALTERED_TIME_RATIO == GEN_0)       ?  1 :
                                                     1 ;
}
...
bool8 RtcCmd_addsecond(struct ScriptContext *ctx)
{
    u32 year = 0;
    u32 month = 0;
    u32 day = 0;
...
bool8 RtcCmd_setsecond(struct ScriptContext *ctx)
{
    struct Time *time = FakeRtc_GetCurrentTime();
