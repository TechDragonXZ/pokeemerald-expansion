...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    else if (gMovesInfo[move].effect == EFFECT_BEAM_BASH)
    {
        if (gBattleMons[battlerAtk].species == SPECIES_TIMBURR)
        {
            gBattleStruct->dynamicMoveType = TYPE_GRASS | F_DYNAMIC_TYPE_SET;
        }
        else if (gBattleMons[battlerAtk].species == SPECIES_GURDURR)
        {
            gBattleStruct->dynamicMoveType = TYPE_STEEL | F_DYNAMIC_TYPE_SET;
        }
        else if (gBattleMons[battlerAtk].species == SPECIES_CONKELDURR)
        {
            gBattleStruct->dynamicMoveType = TYPE_ROCK | F_DYNAMIC_TYPE_SET;
        }
        else
