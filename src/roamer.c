...
void RoamerMoveToOtherLocationSet(u8 index)
{
    ...
    if (ROAMER(index)->isStalker)
    {
        ...
        {
            ROAMER(index)->locationMapGroup = MAP_GROUP(MAP_NONE);
            ROAMER(index)->locationMapNum = MAP_NUM(MAP_NONE);
            return;
        }
        else
        {
...
// The roamer's status field is u8, but SetMonData expects status to be u32
    u32 status = ROAMER(index)->statusA;
    u16 hp;
...
void UpdateRoamerHPStatus(struct Pokemon *mon)
{
    ...
    else
    {
        ...
        ROAMER(gEncounteredRoamerIndex)->statusA = GetMonData(mon, MON_DATA_STATUS);
    }
