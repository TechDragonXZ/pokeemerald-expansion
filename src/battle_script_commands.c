...
static bool32 TryAegiFormChange2(void)
{
    // Only Aegislash with Stance Change can transform, transformed mons cannot.
    if (GetBattlerAbility(gBattlerAttacker) != ABILITY_GHOSTLY_BLADE
        || gBattleMons[gBattlerAttacker].status2 & STATUS2_TRANSFORMED)
        return FALSE;
...
static void Cmd_attackcanceler(void)
{
    ...
    if (B_STANCE_CHANGE_FAIL < GEN_7 && TryAegiFormChange())
        return;
    if (B_STANCE_CHANGE_FAIL < GEN_7 && TryAegiFormChange2())
        return;
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_FROSTBURN:
                ...
                {
                    static const u8 sFrostburnEffects[] =
                    {
                        MOVE_EFFECT_BURN,
                        MOVE_EFFECT_FREEZE_OR_FROSTBITE
                    };
                    gBattleScripting.moveEffect = RandomElement(RNG_TRI_ATTACK, sFrostburnEffects);
                    SetMoveEffect(primary, certain);
                }
                break;
...
void BS_HandleBattleEvo(void)
{
    ...
    if (gLeveledUpInBattle & (1u << (battler))
