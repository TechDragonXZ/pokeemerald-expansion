...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_MAGNITUDE:
            if ((aiData->abilities[battlerDef] == ABILITY_LEVITATE) || (aiData->abilities[battlerDef] == ABILITY_BOOSTED_MOTOR))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_GRAVITY:
            if ((gFieldStatuses & STATUS_FIELD_GRAVITY
              ...
              && (aiData->holdEffects[battlerAtk] != HOLD_EFFECT_AIR_BALLOON || aiData->holdEffects[battlerAtk] != HOLD_EFFECT_UTILITY_PACK)) // Should revert Gravity in this case
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_SNORE:
        ...
        break;
    case EFFECT_LOCK_ON:
        if ((HasMoveEffect(battlerAtk, EFFECT_OHKO)) || HasMoveEffect(battlerAtk, EFFECT_FINAL_BLOW))
            ADJUST_SCORE(GOOD_EFFECT);
