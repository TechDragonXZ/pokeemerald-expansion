...
#define FLAG_CLOCK_MODE         FLAG_CLOCK
...
#if (FLAG_CLOCK_MODE != 0)
    if (FlagGet(FLAG_CLOCK)) // true: 12-hours, false: 24-hours
    {
...
static void Task_StartMenuFullMain(u8 taskId)
{
    ...
    if(JOY_NEW(L_BUTTON)) // If L button pressed open Dexnav
    {
    ...
    {
        if (FlagGet(FLAG_CLOCK))
            FlagClear(FLAG_CLOCK);
        else
            FlagSet(FLAG_CLOCK);
